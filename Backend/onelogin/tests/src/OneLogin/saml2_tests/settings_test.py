...
class OneLogin_Saml2_Settings_Test(unittest.TestCase):
    ...
    def testLoadSettingsFromDict(self):
        ...
        settings_info["idp"][
            "certFingerprint"
        ] = "afe71c28ef740bc87425be13a2263d37971daA1f9"
        settings = OneLogin_Saml2_Settings(settings_info)
        self.assertEqual(len(settings.get_errors()), 0)
        ...
        try:
            ...
            self.assertIn(
                "Invalid dict settings: sp_cert_not_found_and_required", str(e)
            )
    ...
    def testLoadSettingsFromFile(self):
        ...
        custom_base_path = join(
            dirname(__file__), "..", "..", "..", "data", "customPath"
        )
    ...
    def testGetCertPath(self):
        ...
        settings = OneLogin_Saml2_Settings(custom_base_path=self.settings_path)
        self.assertEqual(
            self.settings_path + sep + "certs" + sep, settings.get_cert_path()
        )
    ...
    def testGetSPCert(self):
        ...
        settings_3 = OneLogin_Saml2_Settings(
            settings_data, custom_base_path=custom_base_path
        )
