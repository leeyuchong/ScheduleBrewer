...
class OneLogin_Saml2_Response_Test(unittest.TestCase):
    ...
    def testGetXMLDocument(self):
        ...
        self.assertEqual(
            etree.tostring(response_2.get_xml_document(), encoding="unicode"),
            decrypted,
        )
    ...
    def testReturnNameId(self):
        ...
        response_2 = OneLogin_Saml2_Response(settings, xml_2)
        self.assertEqual(
            "2de11defd199f8d5bb63f9b7deb265ba5c675c10", response_2.get_nameid()
        )
        ...
        with self.assertRaisesRegex(
            Exception, "NameID not found in the assertion of the Response"
        ):
    ...
    def testGetAudiences(self):
        ...
        settings = OneLogin_Saml2_Settings(self.loadSettingsJSON())
        xml = self.file_contents(
            join(self.data_path, "responses", "no_audience.xml.base64")
        )
        response = OneLogin_Saml2_Response(settings, xml)
    ...
    def testQueryAssertions(self):
        ...
        settings = OneLogin_Saml2_Settings(self.loadSettingsJSON())
        xml = self.file_contents(
            join(self.data_path, "responses", "adfs_response.xml.base64")
        )
        response = OneLogin_Saml2_Response(settings, xml)
        self.assertEqual(
            ["http://login.example.com/issuer"], response.get_issuers()
        )
        ...
        xml_3 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "double_signed_encrypted_assertion.xml.base64",
            )
        )
        response_3 = OneLogin_Saml2_Response(settings, xml_3)
        self.assertEqual(
        ...
        xml_5 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "signed_message_encrypted_assertion.xml.base64",
            )
        )
        response_5 = OneLogin_Saml2_Response(settings, xml_5)
        self.assertEqual(
        ...
        xml_7 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "signed_encrypted_assertion.xml.base64",
            )
        )
    ...
    def testGetIssuers(self):
        ...
        response_5 = OneLogin_Saml2_Response(settings, xml_5)
        with self.assertRaisesRegex(
            Exception, "Issuer of the Assertion not found or multiple."
        ):
    ...
    def testGetAttributes(self):
        ...
        xml_2 = self.file_contents(
            join(self.data_path, "responses", "response2.xml.base64")
        )
        response_2 = OneLogin_Saml2_Response(settings, xml_2)
    ...
    def testGetFriendlyAttributes(self):
        ...
        xml_2 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "response1_with_friendlyname.xml.base64",
            )
        )
        response_2 = OneLogin_Saml2_Response(settings, xml_2)
        self.assertEqual(
            expected_attributes, response_2.get_friendlyname_attributes()
        )
    ...
    def testGetNestedNameIDAttributes(self):
        ...
        xml = self.file_contents(
            join(
                self.data_path,
                "responses",
                "response_with_nested_nameid_values.xml.base64",
            )
        )
        response = OneLogin_Saml2_Response(settings, xml)
        expected_attributes = {
    ...
    def testOnlyRetrieveAssertionWithIDThatMatchesSignatureReference(self):
        ...
        settings = OneLogin_Saml2_Settings(self.loadSettingsJSON())
        xml = self.file_contents(
            join(self.data_path, "responses", "wrapped_response_2.xml.base64")
        )
        response = OneLogin_Saml2_Response(settings, xml)
        with self.assertRaisesRegex(
            Exception,
    ...
    def testDoesNotAllowSignatureWrappingAttack(self):
        ...
        settings = OneLogin_Saml2_Settings(self.loadSettingsJSON())
        xml = self.file_contents(
            join(self.data_path, "responses", "response4.xml.base64")
        )
        response = OneLogin_Saml2_Response(settings, xml)
    ...
    def testGetInResponseTo(self):
        ...
        response_3 = OneLogin_Saml2_Response(settings, xml_3)
        self.assertEqual(
            "ONELOGIN_be60b8caf8e9d19b7a3551b244f116c947ff247d",
    ...
    def testIsInvalidXML(self):
        ...
        self.assertEqual(
            "No Signature found. SAML Response rejected", response.get_error()
        )
    ...
    def testValidateNumAssertions(self):
        ...
        xml_multi_assertion = self.file_contents(
            join(
                ...
                "responses",
                "invalids",
                "multiple_assertions.xml.base64",
            )
        )
    ...
    def testValidateTimestamps(self):
        ...
        xml_3 = self.file_contents(
            join(self.data_path, "responses", "expired_response.xml.base64")
        )
        response_3 = OneLogin_Saml2_Response(settings, xml_3)
    ...
    def testValidateVersion(self):
        ...
        xml = self.file_contents(
            join(
                self.data_path, "responses", "invalids", "no_saml2.xml.base64"
            )
        )
        response = OneLogin_Saml2_Response(settings, xml)
    ...
    def testValidateID(self):
        ...
        settings = OneLogin_Saml2_Settings(self.loadSettingsJSON())
        xml = self.file_contents(
            join(self.data_path, "responses", "invalids", "no_id.xml.base64")
        )
        response = OneLogin_Saml2_Response(settings, xml)
        with self.assertRaisesRegex(
            Exception, "Missing ID attribute on SAML Response"
        ):
            response.is_valid(self.get_request_data(), raise_exceptions=True)
    ...
    def testIsValidOptionalStatement(self):
        ...
        # check settings
        self.assertFalse(
            settings.get_security_data()["wantAttributeStatement"]
        )
    ...
    def testIsInValidMultipleAssertions(self):
        ...
        response = OneLogin_Saml2_Response(settings, xml)
        with self.assertRaisesRegex(
            Exception, "SAML Response must contain 1 assertion"
        ):
            response.is_valid(self.get_request_data(), raise_exceptions=True)
    ...
    def testIsInValidDuplicatedAttrs(self):
        ...
        response = OneLogin_Saml2_Response(settings, xml)
        with self.assertRaisesRegex(
            Exception, "Found an Attribute element with duplicated Name"
        ):
    ...
    def testIsInValidDestinationCapitalizationOfElements(self):
        ...
        response = OneLogin_Saml2_Response(settings, message)
        self.assertFalse(
            response.is_valid(self.get_request_data_path_capitalized())
        )
        ...
        response_2 = OneLogin_Saml2_Response(settings, message)
        self.assertFalse(
            response_2.is_valid(self.get_request_data_both_capitalized())
        )
        self.assertIn("The response was received at", response_2.get_error())
    ...
    def testIsValidDestinationCapitalizationOfHost(self):
        ...
        response = OneLogin_Saml2_Response(settings, message)
        self.assertFalse(
            response.is_valid(self.get_request_data_domain_capitalized())
        )
        ...
        self.assertIn(
            "A valid SubjectConfirmation was not found", response.get_error()
        )
    ...
    def testIsInValidAudience(self):
        ...
        self.assertFalse(response_2.is_valid(request_data))
        self.assertIn(
            "is not a valid audience for this Response", response_2.get_error()
        )
    ...
    def testIsInValidAuthenticationContext(self):
        ...
        settings_dict["security"]["requestedAuthnContext"] = [
            two_factor_context
        ]
        ...
        original_message = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(message)
        )
        two_factor_message = original_message.replace(
            password_context, two_factor_context
        )
    ...
    def testIsInValidIssuer(self):
        ...
        plain_message_2 = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(xml_2)
        )
        ...
        with self.assertRaisesRegex(
            Exception, "Invalid issuer in the Assertion/Response"
        ):
    ...
    def testIsInValidSubjectConfirmation(self):
        ...
        plain_message_3 = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(xml_3)
        )
        ...
        plain_message_4 = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(xml_4)
        )
        ...
        plain_message_5 = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(xml_5)
        )
        ...
        plain_message_6 = compat.to_string(
            OneLogin_Saml2_Utils.b64decode(xml_6)
        )
    ...
    def testIsInValidRequestId(self):
        ...
        response = OneLogin_Saml2_Response(settings, message)
        with self.assertRaisesRegex(
            Exception, "The InResponseTo of the Response"
        ):
    ...
    def testIsValid(self):
        ...
        xml = self.file_contents(
            join(
                self.data_path,
                "responses",
                "valid_unsigned_response.xml.base64",
            )
        )
        response = OneLogin_Saml2_Response(settings, xml)
        response.is_valid(self.get_request_data())
    ...
    def testIsValid2(self):
        ...
        settings_2 = OneLogin_Saml2_Settings(settings_info_2)
        xml_2 = self.file_contents(
            join(self.data_path, "responses", "valid_response2.xml.base64")
        )
        ...
        idp_cert = OneLogin_Saml2_Utils.format_cert(
            settings_info_3["idp"]["x509cert"]
        )
        settings_info_3["idp"][
            "certFingerprint"
        ] = OneLogin_Saml2_Utils.calculate_x509_fingerprint(idp_cert)
        settings_info_3["idp"]["x509cert"] = ""
        ...
        settings_info_3["idp"][
            "certFingerprint"
        ] = OneLogin_Saml2_Utils.calculate_x509_fingerprint(idp_cert, "sha256")
    ...
    def testIsValidEnc(self):
        ...
        xml_4 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "double_signed_encrypted_assertion2.xml.base64",
            )
        )
        ...
        xml_5 = self.file_contents(
            join(
                self.data_path,
                "responses",
                "signed_encrypted_assertion2.xml.base64",
            )
        )
        ...
        message = compat.to_string(
            OneLogin_Saml2_Utils.b64encode(plain_message)
        )
    ...
    def testIsValidSign(self):
        ...
        dom_3.firstChild.firstChild.firstChild.nodeValue = (
            "https://example.com/other-idp"
        )
    ...
    def testIsValidSignFingerprint(self):
        ...
        settings = OneLogin_Saml2_Settings(
            self.loadSettingsJSON("settings6.json")
        )
    ...
    def testIsValidWithoutInResponseTo(self):
        ...
        xml = self.file_contents(
            join(
                self.data_path,
                "responses",
                "valid_response_without_inresponseto.xml.base64",
            )
        )
        response = OneLogin_Saml2_Response(settings, xml)

        not_on_or_after = datetime.strptime(
            "2014-02-19T09:37:01Z", "%Y-%m-%dT%H:%M:%SZ"
        )
    ...
    def testGetId(self):
        ...
        response = OneLogin_Saml2_Response(settings, xml)
        self.assertEqual(
            response.get_id(), "pfxc3d2b542-0f7e-8767-8e87-5b0dc6913375"
        )
