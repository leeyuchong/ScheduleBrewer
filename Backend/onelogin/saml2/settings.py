...
class OneLogin_Saml2_Settings(object):
    ...
    def __add_default_values(self):
        ...
        self.__sp["assertionConsumerService"].setdefault(
        ...
        self.__sp["singleLogoutService"].setdefault(
        ...
        # Signature Algorithm
        self.__security.setdefault(
            "signatureAlgorithm", OneLogin_Saml2_Constants.RSA_SHA1
        )
        ...
        if not isinstance(settings, dict) or len(settings) == 0:
            ...
            if not settings.get("idp"):
                ...
                if not idp.get("singleSignOnService", {}).get("url"):
                    ...
                    idp["singleSignOnService"]["url"], allow_single_domain_urls
                ):
            ...
            if not settings.get("sp"):
                ...
                if sp.get("attributeConsumingService"):
                    ...
                    if "serviceName" not in attributeConsumingService:
                        errors.append(
                            "sp_attributeConsumingService_serviceName_not_found"
                        )
                    elif not isinstance(
                        attributeConsumingService["serviceName"], basestring
                    ):
                        errors.append(
                            "sp_attributeConsumingService_serviceName_type_invalid"
                        )
                    ...
                    if "requestedAttributes" not in attributeConsumingService:
                        ...
                        attributeConsumingService["requestedAttributes"], list
                    ):
                        errors.append(
                            "sp_attributeConsumingService_serviceName_type_invalid"
                        )
                    else:
                        for req_attrib in attributeConsumingService[
                            "requestedAttributes"
                        ]:
    ...
    def get_idp_slo_response_url(self):
        ...
        idp_data = self.get_idp_data()
        if "url" in idp_data["singleLogoutService"]:
            return idp_data["singleLogoutService"].get(
                "responseUrl", self.get_idp_slo_url()
            )
    ...
    def get_sp_metadata(self):
        ...
        metadata = self.metadata_class.builder(
            self.__sp,
            self.__security["authnRequestsSigned"],
        ...
        add_encryption = (
            ...
            or self.__security["wantAssertionsEncrypted"]
        )

        cert_new = self.get_sp_cert_new()
        metadata = self.metadata_class.add_x509_key_descriptors(
        ...
        # Sign metadata
        if (
            "signMetadata" in self.__security
            and self.__security["signMetadata"] is not False
        ):
            if self.__security["signMetadata"] is True:
                ...
                if (
                    "keyFileName" not in self.__security["signMetadata"]
                    or "certFileName" not in self.__security["signMetadata"]
                ):
                    raise OneLogin_Saml2_Error(
    ...
    def format_sp_key(self):
        ...
        """
        self.__sp["privateKey"] = OneLogin_Saml2_Utils.format_private_key(
    ...
    def _get_allow_single_label_domain(self, settings):
        security = settings.get("security", {})
        return (
            "allowSingleLabelDomains" in security.keys()
            and security["allowSingleLabelDomains"]
        )
